...
public class AssertingWebElement implements GsiWebElement {
	...
	/**
	 * Un-selects the webelement. This method should be used only for radio buttons.
	 */
	 ...
	 * (non-Javadoc)
	 *
	 * @see com.gsicommerce.webstore.utilities.WebElement#select(java.lang.String)
	 */
	...
	public boolean verifyTextPresentIn(final String expectedText) {
		assertNotNull();
		final String actualText = Normalizer.normalize(wrapped.getText(), Normalizer.Form.NFKC);
		// Common.sleepFor(1000);
	...
	public boolean searchByValue(final String value) {
		assertNotNull();
		final List<WebElement> options = wrapped.findElements(By.xpath(".//option[@value = '" + value + "']"));
	...
	public void selectByPartialText(final String text) {
		...
		for (int i = 0; i < options.size(); i++) {
			if (Common.convertUniCodeToASCII(options.get(i).getText()).toLowerCase().contains(text.toLowerCase())) {
				new Select(wrapped).selectByIndex(i);
				blnFound = true;
				break;
			}
...
//        ((JavascriptExecutor) driver).executeScript(code, wrapped);
		final Actions builder = new Actions(driver); // Or maybe seleniumDriver? Not sure which one to use
