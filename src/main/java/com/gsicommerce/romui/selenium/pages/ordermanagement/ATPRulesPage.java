...
public class ATPRulesPage {
	...
	public boolean verifyInvAvail() throws Exception {
		...
		if (invAvlHeader.getText().contains("Rules Found") && (rowNo > 0) && invOrgHeader.getText().contains("TMSNA_INV_ORG")) {
			found = true;
		}
		return found;
	}
	...
	public void addATPRule() throws Exception
	{
		...
		System.out.println("New ATP Rule Name is::"+inventoryAvailabilityData.getATP_Rule_Name());
		Action.enter(txtbxatpRuleName, inventoryAvailabilityData.getATP_Rule_Name());
		...
		rowATP=CommonElementsPage.getRowNo(inventoryAvailabilityData.getATP_Rule_Name());
		System.out.println("Newly Added ATP Rule Name is:"
					+ CommonElementsPage.getRowCellTextVal(rowATP, 1));
			Assert.assertEquals(CommonElementsPage.getRowCellTextVal(rowATP, 1), inventoryAvailabilityData.getATP_Rule_Name(),
					"New ATP not Added"); 
	}
	...
	{
		inventoryAvailabilityData =InventoryAvailabilityData.get(env.getFileLocation());
	//	int rowATP=CommonElementsPage.getRowNo(inventoryAvailabilityData.getATP_Rule_Name());
