...
import com.gsicommerce.romui.selenium.ROMUI.Utilities.Action;
...
public class Webtable extends Action {
    public GsiWebElement webtable;

    public Webtable(final GsiWebElement element, final WebDriver driver,
    ...
    public boolean exists() {
        if (null == this.webtable || this.webtable.exists() == false) {
            return false;
        } else {
            return true;
        }
    ...
    public int getColumnIndex(final String columnHeader) {
        enableExplicitWait();
        try {
            GsiWebElement rowData;
            final GsiWebElement temp = this.webtable;
    ...
    public boolean findCellData(final String columnHeader, final String expValue) {
        final GsiWebElement cell = getCellData(columnHeader, expValue);
     ...
     */
    public GsiWebElement getCellData(final String columnHeader,
            final String searchValue) {
        disableWait();
        try {
            final GsiWebElement rowData = getRowData(searchValue);
            if (!rowData.exists()) {
     ...
     * @return GsiWebElement
     */
    public GsiWebElement getCellDataUsingExactMatch(final String columnHeader,
            final String searchValue) {
        disableWait();
        try {
            final GsiWebElement rowData = getRowDataUsingExactMatch(searchValue);
            if (!rowData.exists()) {
                return new AssertingWebElement(null);
            }
            final int columnIndex = getColumnIndex(columnHeader);
     ...
     * @return GsiWebElement
     */
    public GsiWebElement getCellData(final GsiWebElement rowData,
     ...
     */
    public GsiWebElement getRowData(final String searchValue) {
        disableWait();
        try {
    ...
    public GsiWebElement getRowDataUsingExactMatch(final String searchValue) {
        disableWait();
        try {
            WebElement rowData = null;
            final List<WebElement> rows = new ArrayList<WebElement>();
     ...
     * @return GsiWebElement
     */
    public GsiWebElement getRowData(final int index) {
        disableWait();
        try {
            final List<WebElement> rows = new ArrayList<WebElement>();
     ...
     */
    public List<GsiWebElement> getColumnValues(final Integer columnIndex) {
        final List<GsiWebElement> valuesList = new ArrayList<GsiWebElement>();
